...
# pydantic 라이브러리를 사용하여 입력 데이터의 규격및 명세를 정의하고 검증(Validation) 
class StockHistoryInput(BaseModel):
    ticker: str = Field(..., title="주식 코드", description="주식 코드 (예: AAPL, 000660.KS)")
...
def get_yf_stock_history(ticker: str, period: str) -> str:
    ...
    stock = yf.Ticker(ticker)  # class TickerBase 클래스
...
# 도구를 모델에 바인딩 openrouter/올라마 나중에 연결
...
messages = [
    ...
    HumanMessage("SK하이닉스의 최근 3일간 주가 정보는 어떻게 되지?")
]                        
